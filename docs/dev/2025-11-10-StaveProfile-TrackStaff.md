...
**0.3.x 版本的问题**：
在原版播放器中，存在一个让人困惑的行为缺陷：
- 初始状态：五线谱和六线谱分离显示
- 选择"仅六线谱"后：仅移除了五线谱视觉元素
- **❌ Bug**：**节拍、时值等乐理元素没有显示到六线谱上**
- 预期行为：六线谱应该包含完整的音乐信息（节奏记号等）

**0.4.x (player 分支) 继承问题**：
- 新播放器的 `StaveProfileControl` 组件直接从旧 `.ts` 代码改写为 `.tsx`
- **未解决原有的功能缺陷**，仍然是"半残"的切换逻辑
- 这是因为误用了 **StaveProfile（全局布局模式）** 而非 **Staff 显示选项（细粒度记谱法控制）**

**正确实现**：
- `TracksPanel` 组件基于官方文档实现，使用 Staff 显示选项
- 可以精细控制每个音轨的 4 种记谱法组合：
  - ✅ 标准记谱法（五线谱）
  - ✅ 吉他谱（六线谱）
  - ✅ 节奏谱（Slash Notation）
  - ✅ 简谱（Numbered Notation）
- 这些选项可以**自由组合**，例如"六线谱+节奏谱"同时显示
