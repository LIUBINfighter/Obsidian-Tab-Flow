...
import { PrintPreviewView, VIEW_TYPE_PRINT_PREVIEW } from './views/PrintPreviewView';
import {
...
export default class TabFlowPlugin extends Plugin {
	...
	async checkRequiredAssets(): Promise<boolean | AssetStatus[]> {
		try {
			...
			// 使用相对路径而非绝对路径
			const assetsDirRelative = joinPath(
			...
			const assetStatuses: AssetStatus[] = await Promise.all(
				assetFiles.map(async (file) => {
					const filePath = joinPath(assetsDirRelative, file);
	...
	async downloadAssets(): Promise<boolean> {
		try {
			...
			const assetsDir = joinPath(this.actualPluginDir, 'assets');
			const assetsDirRelative = joinPath(
				this.app.vault.configDir,
				'plugins',
				this.manifest.id,
	...
	async onload() {
		...
		// 注册打印预览视图
		this.registerView(VIEW_TYPE_PRINT_PREVIEW, (leaf) => new PrintPreviewView(leaf, this));
							...
							void (async () => {
								const parent =
									file instanceof TFile
										? this.app.vault.getAbstractFileByPath(dirName(file.path))
										: file;
								const baseName = t(
