...
export class TabView extends FileView {
	...
	public getScoreTitle(): string {
		if (
			this.scoreTitle &&
			this.scoreTitle.trim() &&
			!isMessy(this.scoreTitle)
		) {
			return this.scoreTitle;
		}
	...
	private async _setupAudioIntegrationInternal(): Promise<void> {
		...
		if (!this.externalMediaService) {
			this.externalMediaService = new ExternalMediaService(
				this._api,
				this.eventBus
			);
		} else {
		...
		try {
			...
			if (
				this.audioObjectUrl &&
				this.audioObjectUrl.startsWith("blob:")
			) {
		...
		const playBar = createPlayBar({
			...
			getCurrentTime: () =>
				this._api?.tickPosition !== undefined && this._api.score
					...
					: 0,
			getDuration: () =>
				this._api?.score ? (this._api.score as any).duration || 0 : 0,
			seekTo: (ms) => {
				if (this._api) {
		...
		if (this._api && this._api.playerPositionChanged) {
			this._api.playerPositionChanged.on((args) => {
				window.requestAnimationFrame(() => {
					const progressFill = playBar.querySelector(
						".progress-fill"
					) as HTMLElement;
					const progressHandle = playBar.querySelector(
						".progress-handle"
					) as HTMLElement;
					const currentTimeDisplay = playBar.querySelector(
						".current-time"
					) as HTMLElement;
					const totalTimeDisplay = playBar.querySelector(
						".total-time"
					) as HTMLElement;
		...
		this.eventBus.subscribe("命令:加载当前文件", async () => {
			...
			try {
				if (
					...
					["alphatab", "alphatex"].includes(
						this.currentFile.extension.toLowerCase()
					)
				) {
					const textContent = await this.app.vault.read(
						this.currentFile
					);
					this.eventBus.publish("命令:加载AlphaTex乐谱", textContent);
				} else {
					const inputFile = await this.app.vault.readBinary(
						this.currentFile
					);
	...
	async onLoadFile(file: TFile): Promise<void> {
		...
		try {
			...
			if (
				file.extension &&
				["alphatab", "alphatex"].includes(file.extension.toLowerCase())
			) {
	...
	private configureScrollElement(): void {
		...
		setTimeout(() => {
			if (this._api.settings.player) {
				this._api.settings.player.scrollMode =
		...
		if (scrollElement) {
			this.horizontalScrollCleanup = setupHorizontalScroll(
				scrollElement,
				this._api
			);
		}
