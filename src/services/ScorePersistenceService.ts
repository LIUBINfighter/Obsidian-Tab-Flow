...
export class ScorePersistenceService {
	...
	public loadScoreViewState(
		...
		try {
			const raw = localStorage.getItem(
				this.localStorageKeyPrefix + filePath
			);
			if (!raw) return;
			...
			if (
				...
				allTracks.forEach((track) => {
					const s =
						settings[String(track.index)] || settings[track.index];
					if (!s) return;
					try {
						...
						if (typeof s.transposeAudio === "number") {
							...
							(track.playbackInfo as any).transposeAudio =
								s.transposeAudio;
						}
					} catch (e) {
						console.warn(
	...
	public saveSelectedTracks(
		...
		try {
			...
			let state: ScoreViewState = raw ? JSON.parse(raw) : {};
	...
	public saveTrackSettings(
		...
		try {
			...
			state.trackSettings[key] = Object.assign(
				{},
				state.trackSettings[key] || {},
				partial
			);
			localStorage.setItem(
				this.localStorageKeyPrefix + filePath,
