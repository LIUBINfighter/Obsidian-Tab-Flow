 ...
 * 类型守卫：判断是否为合成器模式
 */
export function isSynthMedia(
	source: MediaSource
): source is Extract<MediaSource, { type: MediaType.Synth }> {
...
export function isAudioMedia(
	source: MediaSource
): source is Extract<MediaSource, { type: MediaType.Audio }> {
...
export function isYouTubeMedia(
	source: MediaSource
): source is Extract<MediaSource, { type: MediaType.YouTube }> {
 ...
 * 类型守卫：判断是否为外部媒体模式
 */
export function isExternalMedia(
	source: MediaSource
): source is Extract<MediaSource, { type: MediaType.External }> {
 ...
 * 从音频 URL 创建媒体源
 */
export function createAudioMediaSource(
	url: string,
	blob?: Blob,
	audioFile?: Uint8Array
): MediaSource {
