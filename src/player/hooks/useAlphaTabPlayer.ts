...
export function useAlphaTabPlayer(
    ...
    useEffect(() => {
        ...
        if (config.layoutMode !== undefined && api.settings.display.layoutMode !== config.layoutMode) {
            api.settings.display.layoutMode = config.layoutMode;
            needsUpdate = true;
        }
        
        if (config.staveProfile !== undefined && api.settings.display.staveProfile !== config.staveProfile) {
            api.settings.display.staveProfile = config.staveProfile;
            needsUpdate = true;
        }
        
        if (config.scrollMode !== undefined && api.settings.player.scrollMode !== config.scrollMode) {
            api.settings.player.scrollMode = config.scrollMode;
            needsUpdate = true;
        }
        
        if (config.scrollOffsetY !== undefined && api.settings.player.scrollOffsetY !== config.scrollOffsetY) {
            api.settings.player.scrollOffsetY = config.scrollOffsetY;
            needsUpdate = true;
        }
        
        if (config.playerMode !== undefined && api.settings.player.playerMode !== config.playerMode) {
