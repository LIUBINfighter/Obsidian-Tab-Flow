...
export class TracksModal extends Modal {
	...
	onOpen() {
			...
			if (filePath) {
				...
				if (raw) {
					...
					if (state && Array.isArray(state.tracks) && this.tracks) {
						...
						if (matched.length > 0)
							this.selectedTracks = new Set(matched);
					}
					// 恢复轨道参数
					if (
						state &&
						state.trackSettings &&
						typeof state.trackSettings === "object"
					) {
						savedTrackSettings = state.trackSettings as Record<
							string,
							any
						>;
					}
		...
		this.tracks.forEach((track) => {
				...
				btn.onClick(() => {
					...
					if (this.api) {
						...
						if (filePath && this.scorePersistenceService) {
							this.scorePersistenceService.saveTrackSettings(
								filePath,
								track.index,
								{ solo: newSolo }
							);
			...
			try {
				const s = savedTrackSettings?.[String(track.index)];
				const initTa =
					typeof s?.transposeAudio === "number"
						? s.transposeAudio
						: 0;
	...
	private onSelectTrack() {
		...
		if (this.api && this.scorePersistenceService) {
			const filePath = (this.api as any)?.score?.filePath;
			if (filePath) {
				this.scorePersistenceService.saveSelectedTracks(
					filePath,
					selectTracks
				);
			}
