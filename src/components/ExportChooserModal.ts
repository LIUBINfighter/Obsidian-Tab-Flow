...
export class ExportChooserModal extends Modal {
    ...
    onOpen(): void {
        ...
        // 文件名编辑框
        const nameWrap = contentEl.createDiv({ attr: { style: "display:flex; align-items:center; gap:8px; margin-bottom:8px;" } });
        ...
        audioBtn.onclick = () => {
            try {
                ...
                const okHandler = (url?: string) => {
                    onOk(url);
                    try { this.eventBus.unsubscribe?.("状态:音频导出完成", okHandler); } catch {}
                    try { this.eventBus.unsubscribe?.("状态:音频导出失败", failHandler); } catch {}
                };
